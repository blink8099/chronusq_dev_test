...
namespace ChronusQ {
  ...
  void AOIntegrals<IntsT>::twoBodyContractIncore(
    MPI_Comm comm, std::vector<TwoBodyContraction<MatsT>> &list) {
  ...
  template <typename IntsT>
  template <typename MatsT>
  void AOIntegrals<IntsT>::twoBodyContractSOCIncore(
    MPI_Comm comm, std::vector<TwoBodyContraction<MatsT>> &list) {
    ...
    for(auto &C : list) {
      ...
      // Coulomb-type (34,12) ERI contraction
      // AX(mn) = (mn | kl) X(kl)
      if( C.contType == COULOMB ) JContractSOCIncore(comm,C);

      // Exchange-type (23,12) ERI contraction
      // AX(mn) = (mk |ln) X(kl)
      else if( C.contType == EXCHANGE ) KContractSOCIncore(comm,C);
...
#ifdef _REPORT_INTEGRAL_TIMINGS
    std::cerr << "SOC Incore Contraction took " << durIncore.count() << " s\n\n";
#endif
  }; // AOIntegrals::twoBodyContractIncore
  ...
  template <typename IntsT>
  template <typename MatsT>
  void AOIntegrals<IntsT>::JContractIncore(MPI_Comm comm, TwoBodyContraction<MatsT> &C) {
    ...
    if( std::is_same<IntsT,dcomplex>::value )
      ...
      Gemm('N','N',nSQ_,1,nSQ_,IntsT(1.),ERI,nSQ_,X,nSQ_,IntsT(0.),AX,nSQ_);
  ...
  template <typename IntsT>
  template <typename MatsT>
  void AOIntegrals<IntsT>::JContractSOCIncore(MPI_Comm comm, TwoBodyContraction<MatsT> &C) {
    ...
    for(auto l = 0; l < basisSet_.nBasis; ++l) 
      ...
      C.AX[i + j*basisSet_.nBasis] +=ERISOC[i + j*basisSet_.nBasis + l*nSQ_ + k*NB3] *
        C.X[k + l*basisSet_.nBasis];

    #else


    Gemm('C','N',nSQ_,1,nSQ_,IntsT(1.),ERISOC,nSQ_,X,nSQ_,IntsT(0.),AX,nSQ_);
  ...
  template <typename IntsT>
  template <typename MatsT>
  void AOIntegrals<IntsT>::KContractIncore(MPI_Comm comm, TwoBodyContraction<MatsT> &C) {
    ...
    for(auto nu = 0; nu < NB; nu++) 
      Gemm('N','N',NB,1,nSQ_,MatsT(1.),ERI+nu*NB3,NB,C.X,nSQ_,MatsT(0.),C.AX+nu*NB,NB);
  ...
  template <typename IntsT>
  template <typename MatsT>
  void AOIntegrals<IntsT>::KContractSOCIncore(MPI_Comm comm, TwoBodyContraction<MatsT> &C) {
    ...
    for(auto l = 0; l < basisSet_.nBasis; ++l) {
      C.AX[i + j*basisSet_.nBasis] +=
        ERISOC[i + k*basisSet_.nBasis + l*nSQ_ + j*NB3] *
        C.X[k + l*basisSet_.nBasis];
    }
    ...
    for(auto nu = 0; nu < basisSet_.nBasis; nu++) 
      Gemm('N','N',basisSet_.nBasis,1,nSQ_,MatsT(1.),
        ERISOC  + nu * NB3,              basisSet_.nBasis,
        C.X,                          nSQ_,MatsT(0.),
