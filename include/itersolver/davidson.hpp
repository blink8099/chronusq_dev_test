...
namespace ChronusQ {
  ...
  bool Davidson<_F>::runMicro() {
    ...
    if( isRoot ) {
      ...
      Ovlp   = this->memManager_.template malloc<_F>(nG2); 
	...
	for(auto i = 0; i < nExam; i++) {
	    std::cout << "        Root " << std::setw(5) << std::right << i+1 << ":"
	...
	if( iter > 0) {
	  ...
	  for(i = 0; i < nExam; i++) {
	      ...
	      continue; 
	    }else if (j!=i) std::cout << "          New root" << std::setw(5) << i
	    ...
	    if(SiConv[i]) std::cout << "        Root " << std::setw(5) << std::right << i+1
		                    << " has converged"  << std::endl;
	    else std::cout << "        Root " <<  std::right << std::setw(5) << i+1
	...
	std::fill_n(EigForT,nG,dcomplex(0.));
