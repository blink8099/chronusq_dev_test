...
namespace ChronusQ {
  ...
  void Integrals<IntsT>::computeAOOneE(CQMemManager &mem,
    ...
    for (const std::pair<OPERATOR,size_t> &op : ops)
      switch (op.first) {
      case OVERLAP:
        overlap = std::make_shared<OneEInts<IntsT>>(mem, NB);
        overlap->computeAOInts(basis, mol, emPert, OVERLAP, options);
        if( savFile.exists() )
          savFile.safeWriteData("INTS/OVERLAP", overlap->pointer(), {NB,NB});
        break;

      case KINETIC:
        kinetic = std::make_shared<OneEInts<IntsT>>(mem, NB);
        kinetic->computeAOInts(basis, mol, emPert, KINETIC, options);
        if( savFile.exists() )
          savFile.safeWriteData("INTS/KINETIC", kinetic->pointer(), {NB,NB});
        break;

      case NUCLEAR_POTENTIAL:
        // Use Libint for point nuclei, in-house for Gaussian nuclei
        if (options.OneEScalarRelativity)
          potential = std::make_shared<OneERelInts<IntsT>>(
              mem, NB, options.OneESpinOrbit);
        else
          potential = std::make_shared<OneEInts<IntsT>>(mem, NB);
        ...
        if( savFile.exists() ) {
          ...
          savFile.safeWriteData("INTS/POTENTIAL" + potentialTag,potential->pointer(),{NB,NB});
        }
        break;
      ...
      case ELECTRON_REPULSION:
        CErr("Electron repulsion integrals are not implemented in computeAOOneE,"
