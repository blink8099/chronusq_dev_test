...
namespace ChronusQ {
  ...
  void GTODirectERIContraction<MatsT,IntsT>::directScaffold(
...
//      prettyPrintSmart(std::cerr,"AX in Direct",C.AX,NB,NB,NB);
  ...
  template <>
  void GTODirectERIContraction<dcomplex,dcomplex>::directScaffold(
      MPI_Comm comm, const bool screen,
      std::vector<TwoBodyContraction<dcomplex>> &list, EMPerturbation &pert) const {

    DirectERI<dcomplex> &eri =
        dynamic_cast<DirectERI<dcomplex>&>(this->ints_);
    CQMemManager& memManager_ = eri.memManager();
    BasisSet& basisSet_ = eri.basisSet();
...
//      prettyPrintSmart(std::cerr,"AX in Direct",C.AX,NB,NB,NB);
  ...
  template <>
  void GTODirectERIContraction<double,double>::directScaffold(
  ...
  template <>
  void GTODirectERIContraction<dcomplex,double>::directScaffold(
    MPI_Comm c, const bool b, 
    std::vector<TwoBodyContraction<dcomplex>> &list, EMPerturbation &pert) const {
    CErr("GIAO + Real is an invalid option",std::cout);  
  }
  ...
  // New Direct Code
  template <typename MatsT, typename IntsT>
  void GTODirectERIContraction<MatsT,IntsT>::directScaffoldNew(
    MPI_Comm comm, const bool screen,
...
//      nMat*nShell*nShell*sizeof(double);
    ...
    engines[0].set_precision(
      std::max(
        std::numeric_limits<double>::epsilon(),
        eri.threshSchwartz()/(maxShBlkNorm*maxnPrim4))
      );
#endif
...
//      prettyPrintSmart(std::cerr,"AX in Direct",C.AX,nBasis,nBasis,nBasis);
  ...
  void DirectGradContraction<MatsT,IntsT>::directScaffoldGrad(
    ...
    DirectERI<IntsT> &eri =
        dynamic_cast<DirectERI<IntsT>&>(*this->grad_[0]);
    CQMemManager& memManager_ = eri.memManager();
    BasisSet& basisSet_ = eri.basisSet();
    ...
    for( auto iTh  = 0; iTh < nThreads; iTh++) {
      ...
      if( cList[iGrad][iMat].HER ) {
        ...
        if( nThreads != 1 )
          ...
          Scale(nBasis*nBasis,MatsT(0.5),cList[iGrad][iMat].AX,1);
