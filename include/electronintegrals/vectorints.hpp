...
namespace ChronusQ {
  ...
  class VectorInts : public ElectronIntegrals {
  ...
  public:
    ...
    VectorInts(CQMemManager &mem, size_t nb, size_t order, bool symm):
        ElectronIntegrals(mem, nb), order_(order), symmetric_(symm) {
      if (order == 0)
    ...
    virtual void output(std::ostream &out, const std::string &s = "",
      ...
      if (printFull) {
        ...
        if (s == "")
          ...
          oeiStr = "VectorInts[" + s + "].";
        for (size_t i = 0; i < size(); i++)
          prettyPrintSmart(out, oeiStr+indexToLabel(i, order(), symmetric()),
