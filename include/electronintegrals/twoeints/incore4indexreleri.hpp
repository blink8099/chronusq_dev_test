...
namespace ChronusQ {
  ...
  class InCore4indexRelERI : public InCore4indexERI<IntsT> {

    template <typename IntsU>
    friend class InCore4indexRelERI;

  protected:
    std::vector<InCore4indexERI<IntsT>> components_;
  ...
  public:
    ...
    InCore4indexERI<IntsT>& operator[](size_t i) {
      if (i >= components_.size())
        CErr("Requested component is NOT in this RelativisticInts object.");
      return components_[i];
    }

    InCore4indexERI<IntsT>& operator[](REL_INTS_COMPS comp) {
      if (comp == REL_INTS_COMPS::O)
        return *this;
      return operator[](static_cast<size_t>(comp));
    }

    const InCore4indexERI<IntsT>& operator[](size_t i) const {
      if (i >= components_.size())
        CErr("Requested component is NOT in this RelativisticInts object.");
      return components_[i];
    }

    const InCore4indexERI<IntsT>& operator[](REL_INTS_COMPS comp) const {
      if (comp == REL_INTS_COMPS::O)
        return *this;
      return operator[](static_cast<size_t>(comp));
    }

    std::vector<InCore4indexERI<IntsT>>& relComps() {
      return components_;
    }
    const std::vector<InCore4indexERI<IntsT>>& relComps() const {
      return components_;
    }
	...
	// Use Libint to compute nonrelativistic
        InCore4indexERI<IntsT>::computeAOInts(basisSet, mol, emPert, op, hamiltonianOptions);
  ...
  // Use Libint to compute DCB integrals
    ...
    virtual void clear() {
      ...
      for (InCore4indexERI<IntsT>& c : components_)
        c.clear();
    }
    ...
    dcomplex, double>::type> transform(
      ...
      dcomplex, double>::type> transInts(
          this->ElectronIntegrals::memManager(), NT, nRelComp());
  ...
  class InCore4indexRelERIContraction : public InCore4indexERIContraction<MatsT,IntsT> {
  ...
  public:
    ...
    InCore4indexRelERIContraction(TwoEInts<IntsT> &eri):
