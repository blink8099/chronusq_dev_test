...
namespace ChronusQ {
  ...
  class OneERelInts : public OneEInts<IntsT> {

    template <typename IntsU>
    friend class OneERelInts;
  ...
  public:
    ...
    // Constructor
    OneERelInts() = delete;
    OneERelInts( const OneERelInts & ) = default;
    OneERelInts( OneERelInts && ) = default;
    OneERelInts(CQMemManager &mem, size_t nb, bool SORelativistic):
        OneEInts<IntsT>(mem, nb) {
    ...
    template <typename IntsU>
    OneERelInts( const OneERelInts<IntsU> &other, int = 0 ):
        OneEInts<IntsT>(other, 0) {
      if (std::is_same<IntsU, dcomplex>::value
          and std::is_same<IntsT, double>::value)
        CErr("Cannot create a Real OneERelInts from a Complex one.");
    ...
    OneEInts<IntsT>& operator[](REL_INTS_COMPS comp) {
    ...
    const OneEInts<IntsT>& operator[](REL_INTS_COMPS comp) const {
      if (comp == REL_INTS_COMPS::O)
        return *this;
      size_t i = static_cast<size_t>(comp);
      if (i >= components_.size())
    ...
    OneEInts<IntsT>& scalar() { return operator[](REL_INTS_COMPS::SCALAR); }
    const OneEInts<IntsT>& scalar() const { return operator[](REL_INTS_COMPS::SCALAR); }
    OneEInts<IntsT>& SOX() { return operator[](REL_INTS_COMPS::SOX); }
    const OneEInts<IntsT>& SOX() const { return operator[](REL_INTS_COMPS::SOX); }
    OneEInts<IntsT>& SOY() { return operator[](REL_INTS_COMPS::SOY); }
    const OneEInts<IntsT>& SOY() const { return operator[](REL_INTS_COMPS::SOY); }
    OneEInts<IntsT>& SOZ() { return operator[](REL_INTS_COMPS::SOZ); }
    const OneEInts<IntsT>& SOZ() const { return operator[](REL_INTS_COMPS::SOZ); }

    std::vector<OneEInts<IntsT>>& SZYX() {
      return components_;
    }
    const std::vector<OneEInts<IntsT>>& SZYX() const {
      return components_;
    }
    ...
    virtual void output(std::ostream &out, const std::string &s = "",
      ...
      if (printFull) {
        ...
        std::string oeiStr;
        if (s == "")
          oeiStr = "Relativistic one-electron integral";
        else
          oeiStr = "RelOEI[" + s + "]";
        out << oeiStr;
        if(this->hasSpinOrbit())
    ...
    OneERelInts<typename std::conditional<
    ...
    dcomplex, double>::type> transform(
      ...
      dcomplex, double>::type> transInts(
          this->ElectronIntegrals::memManager(), NT, hasSpinOrbit());
