...
namespace ChronusQ {
   ...
   */
  template <>
  void InCoreRIERI<dcomplex>::contract2CenterERI(dcomplex*) {
    CErr("Only real GTOs are allowed",std::cout);
  };
  template <>
  void InCoreRIERI<double>::contract2CenterERI(double *S) {
  ...
  void computeDiagonalPrebuilt4Index(double *diag,
      std::shared_ptr<InCore4indexERI<double>> eri4I_){
  ...
  inline bool computeDynamicAllERIVectorCopyCases(
      ...
      std::map<std::pair<size_t, size_t>, double*> &diagBlocks,
      std::shared_ptr<InCore4indexERI<double>> eri4I) {
  ...
  void computeDynamicAllERIVectorLibcint(
      ...
      double tau, size_t NB, std::shared_ptr<InCore4indexERI<double>> eri4I,
